...
class PingClient(LeotestRunner):
    ...
    def _start_test(self, artifact_name):
        ...
        if shutil.which("ping") is not None:
            ...
            interval = self._config["tests"][self.name]["interval"]
            num_pings = self._config["tests"][self.name]["num_pings"]
            timeout = self._config["tests"][self.name]["test_length"]

            if num_pings:
                starttime, endtime, output = execute("ping", "-D", "-i", str(interval), "-c", str(num_pings), ip)
            else:
                starttime, endtime, output = execute("ping", "-D", "-i", str(interval), "-w", str(timeout), ip)
            ...
            # write curl ipconfig.io output
            _, _, ipv4_output = execute("curl", "-4", "ipconfig.io")

            ip = os.path.join(self.artifact_path, "ip.log")

            with open(ip, 'w', encoding='utf-8') as f:
                f.write(ipv4_output.stdout + "\n")
